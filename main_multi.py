...
BUFFER_SIZE =   1000000   # int(1e6) # Replay buffer size
...
def main():
    ...
    env = envs.make_parallel_env(parallel_envs, SCENARIO, seed = SEED+PRE_TRAINED_EP, num_agents=num_agents, num_landmarks=num_landmarks, benchmark = BENCHMARK)
    ...
    for episode in range(0, number_of_episodes, parallel_envs):
            ...
            if BENCHMARK:
                ...
                for e, inf in enumerate(info):
                    for l in range(num_landmarks):
                        error_mean[l] = np.add(error_mean[l],(inf['n'][l][0]))
        ...
        if episode % 100 < parallel_envs or episode == number_of_episodes-1:
